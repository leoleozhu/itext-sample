...
public class CustomCircleSvgNodeRenderer extends AbstractSvgNodeRenderer {
    ...
    private boolean setParameters(SvgDrawContext context) {
        ...
        if (getAttribute(SvgConstants.Attributes.R) != null) {
            r = parseAbsoluteLength(getAttribute(SvgConstants.Attributes.R), context.getCurrentViewPort().getHeight(), 0.0f, context);
        } else {
    ...
    public Rectangle getObjectBoundingBox(SvgDrawContext svgDrawContext) {
        return this.setParameters(svgDrawContext) ? new Rectangle(this.cx - this.r, this.cy - this.r, this.r , this.r * 2) : null;
    }
